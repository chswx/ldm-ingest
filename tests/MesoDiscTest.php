...
use chswx\LDMIngest\Parser\ProductTypes\MesoDisc;
...
class MesoDiscTest extends \PHPUnit\Framework\TestCase
{
    ...
    private function createMesoDisc(string $productText, string $office = 'KWNS', string $pil = 'SWOMCD'): MesoDisc
    {
        $prodInfo = [
            'office' => $office,
            'pil' => $pil,
            'timestamp' => strtotime('2026-06-26 23:25:00 UTC'),
        ];
    ...
    public function testPilPropagatedToSegment(): void
    {
        $text = $this->loadSampleData('SWOMCD-severe-potential.txt');
        $disc = $this->createMesoDisc($text, 'KWNS', 'SWOMCD');
        $segments = $disc->parse();
        $this->assertEquals('SWOMCD', $segments[0]['pil']);
    }
    ...
    public function testPolygonWithThreeCoordinatesReturnsPolygon(): void
    {
        ...
        $this->assertNotNull($polygon);
        $this->assertCount(4, $polygon['coordinates'][0]);
        $this->assertSame($polygon['coordinates'][0][0], $polygon['coordinates'][0][3]);
    }
